...
@Component({
  selector: 'lib-return-cpio',
  templateUrl: './return-cpio.component.html',
  styleUrls: ['./return-cpio.component.css']
})
export class ReturnCpioComponent  {
  highlighted;
  ...
  ngOnInit() {
    ...
    // this.License='11d74c5c-a928-4bde-a545-501683cccd1c';
...
this.apiService.updateBid_Result(this.bidResult).subscribe((res:any)=>{
  ...
  this.fetchDataa(bid.bid_No,bid.title_Deed_No)
  this.Form=false
},
(error) => {
  console.log(error);
...
(error) => {
  ...
  resetForm() {
    ...
    this.bidResult = {
      ...
      status: null,
      app_Code: null,
      remark: null,is_CPO_Returned:null
    };
    // this.bidResult.bid_No=this.Public_bid_No
    this.isEdit=false
  }
